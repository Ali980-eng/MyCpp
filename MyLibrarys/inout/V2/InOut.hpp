...
namespace io
{
     ...
     */
    template <typename T>
    void print_test_vec(long TNum, const std::vector<T> &vec)
    {
        if (vec.empty())
            throw std::invalid_argument("The vector is empty");
     ...
     */
    void PrintChStr(const std::string &str, short index = 0) noexcept
    {
     ...
     */
    void PrintCategory(const std::string &category) noexcept
    {
