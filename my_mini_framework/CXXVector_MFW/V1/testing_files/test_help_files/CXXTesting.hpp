#pragma once
#ifndef PROGRAMING_C___PROGRAMMINGLIBRARYS_MYLIBRARYS_LAB_CXXTESTING_HPP
#define PROGRAMING_C___PROGRAMMINGLIBRARYS_MYLIBRARYS_LAB_CXXTESTING_HPP
#include "CXXIO.hpp"
#include <memory>
namespace test
{
     ...
     */
    template <typename T>
    bool basic(T realvalue, T expvalue, bool detalis = false,
               int seplen = 25, int white_space = 1, char sepch = '-') noexcept
    {
     ...
     * @endcode
     */
    bool basic(std::string realvalue, std::string expvalue, bool detalis = false,
               int seplen = 25, int white_space = 1, char sepch = '-') noexcept
    {
     ...
     */
    template <typename T>
    bool vector(std::vector<T> real_v, std::vector<T> expected_v, bool detalis = false,
                int seplen = 25, int white_space = 1, char sepch = '~')
    {
     ...
     */
    template <typename T>
    bool vector_2d(std::vector<std::vector<T>> real_2dv, std::vector<std::vector<T>> expected_2dv,
                   bool detalis = false, int splen = 25, int white_space = 1, char sepch = '=')
    {
    ...
    {
        index.function++;
        return basic<T>(fx(), expected, detalis, seplen, white_space);
    }
    /**
    ...
    {
        index.function++;
        return basic<T>(fx(value), expected, detalis, seplen, white_space);
    }
    /**
     * @brief Tests a function by comparing its output to an expected value.
     *
    ...
    {
        index.function++;
        return basic<T1>(fx(value), expected, detalis, seplen, white_space);
    }
    /**
    ...
    {
        index.function++;
        return basic<T1>(fx(value1, value2), expected, detalis, seplen, white_space);
    }
    /**
...
#endif // PROGRAMING_C___PROGRAMMINGLIBRARYS_MYLIBRARYS_LAB_CXXTESTING_HPP